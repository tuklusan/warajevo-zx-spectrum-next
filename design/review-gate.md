...
- Reviewer reasoning profile is recorded by `tools/reviewer/review-profile-v4.json`: health/pure formatting `none`; CODE/DOCUMENTATION/TEST discovery `medium`; integration/falsification/adjudication `high`. The profile's `reasoning_budget` values are local audit metadata only because NVIDIA's current V2 runner rejects the corresponding wire parameter; the selected `reasoning_effort` and `max_tokens` are transmitted. HTTP 202 is polled by requestId; transport retry and schema/semantic failure are separate.
- Any `OutputError`, schema-invalid response, or malformed provider envelope is a packet-construction stop, not a reviewer finding. The gate returns `packet_audit_required: true` with immutable snapshot, packet-manifest, requirements-manifest, source-hash, and revalidation identities; it must not automatically resubmit until requirement, related excerpt, and diff bindings have been re-audited.
