...
443. [P16][Core §43] Establish performance baselines for CPU execution, raster generation, audio mixing, tape, snapshots, and UI presentation before optimization — original pre-optimization baselines were captured on `574714b` (run `35886051010`); the accepted raster optimization was recalibrated on `3501819` (run `35888370795`). The attempted four-runner refresh on `b509ee8` (run `35936294325`) failed because all runners reported a raster fingerprint mismatch and is not accepted. The latest four-runner comparison passed all six workloads with matching deterministic fingerprints on source `0aa7958f0a3d45c86d86507de2423087191f6ea1` (run `35955514283`); all throughputs stayed within the 40% slowdown limit. The lowest rate was macOS Intel UI presentation at 38.7% below baseline; retain it as a remeasurement watch item. CPU and snapshot fingerprint expectations were refreshed for state format v14 while historical throughput measurements remain unchanged.
...
445. [P16][Core §43] Optimize one subsystem at a time with before/after canonical state/raster/audio regression evidence — the raster projection remains supported by runs `35886051010` and `35888370795`. Updated AY timing and state persistence passed canonical regression run `35955514192`; six benchmark fingerprints passed on all four runners in run `35955514283`.
446. [P16][Core §§7] Reject optimizations that introduce undefined behavior, host-width assumptions, or compiler-specific deterministic results — completed for the raster projection, timed ULA capture, and AY envelope progression: bounds are checked before indexing, invalid profiles are rejected before machine state or allocations change, storage is heap allocated and released, and all eight compiler/platform proofs passed on source `0aa7958f0a3d45c86d86507de2423087191f6ea1` (run `35955514187`) with metadata and fixture hashes verified against pinned Git objects.
447. [P16][Core §§6,31] Reject optimizations that make machine results depend on thread scheduling or host wall clock — completed: raster flash phase, ULA capture, and AY tone/noise/envelope progression derive from machine-profile and master-tick inputs; all eight compiler/platform regression proofs passed on run `35955514187` and four-runner metric fingerprints matched on run `35955514283`.
448. [P16][Core §43] Add performance regressions/benchmarks for accepted optimizations so later changes do not silently erase gains — completed: the hosted workflow checks six workload fingerprints and runner throughput against the pinned four-runner baseline. All fingerprints matched and all throughputs remained within the 40% slowdown limit on run `35955514283`; the lowest rate was macOS Intel UI presentation at 38.7% below baseline.
449. [P16][Core §49.1] Close the Phase-16 gate when every optimization in the release has correctness evidence — completed for the implemented Phase-16 scope: canonical regression run `35955514192`, eight compiler/platform proofs on run `35955514187`, and four-runner six-workload performance comparison on run `35955514283` passed; earlier before/after raster evidence remains pinned in run `35888370795`.

## Final Architecture-#1/#2 acceptance and release-readiness sweep

450. [FINAL][Core §50][AUDITED; gaps remain] Inventory all 75 criteria and record conservative evidence status in [acceptance-evidence-audit.md](acceptance-evidence-audit.md); 11 PASS, 12 PARTIAL, and 52 OPEN. The audit is complete; architecture completion remains gated on resolving the OPEN criteria.
...
455. [FINAL][Core §§36–40][AUDITED; gate open] Deterministic, video, audio, differential, private-media, and hardware evidence remains incomplete as recorded in [final release-readiness audit](final-release-readiness-audit.md) and the criterion audits. Updated canonical regression passed ULA schedule, full-frame 192 × 32 fetch/address/value and floating-bus coverage, full 448 × 312 border-raster projection, non-boundary tick checks, timed write/fetch ordering, blanking, border timing, profile-capacity rejection, FLASH/BRIGHT rendering, beeper/AY timing, envelope/noise/mixer behavior, divider boundaries, and state round-trip preservation on run `35955514192`, source `0aa7958`; all eight compiler/platform proofs passed on run `35955514187`, with fixture hashes verified. Four-runner performance passed all six metric checks on run `35955514283`; every runner stayed within the 40% limit, with the lowest rate macOS Intel UI presentation at 38.7% below baseline. The Sokol host paces complete profile-sized master-tick frames, applies runtime speed changes at host pacing anchors, and leaves core clock ratios unchanged. Host audio now renders timestamped beeper transitions with AY register-event replay and submits canonical PCM to Sokol for enabled speeds. DIZZY4K smoke run `35959590777` completed on all four runners at source `01169546836fe92238656e3529d3a1af095a2efb`; it verifies host build/runtime and screenshot capture, not delivered sound. Mandatory source review flagged an apparent uninitialized PCM read; adjudication confirmed `wz_beeper_render_pcm` fills each consumed sample before `wz_audio_mixer_sample` reads it. Exact speed-ratio, audio-delivery, and master-tick continuity regressions remain open. Criteria 18 and 25 pass; criteria 15–17 and 19–20 remain PARTIAL. Complete video/audio suites, Warajevo differential coverage, private difficult-media completion, and real-hardware/reference certification remain open.
...
460. [FINAL][Core §49][UI §§49–50][AUDITED; gate open] The core audit records 11 PASS, 12 PARTIAL, and 52 OPEN criteria; the UI audit records 0 PASS, 7 PARTIAL, and 60 OPEN. Tasks 452–453 and 455–457 retain implementation, evidence, or release-gate gaps. See [final release-readiness audit](final-release-readiness-audit.md), [acceptance-evidence-audit.md](acceptance-evidence-audit.md), and [ui-acceptance-evidence-audit.md](ui-acceptance-evidence-audit.md).
