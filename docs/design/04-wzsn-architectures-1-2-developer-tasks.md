...
060. [P2][Core §36.1][COMPLETE] Integrated the complete pinned Fuse Z80 suite into `wz_tests`; all 1,356 cases passed on hosted run `35918957994` at source commit `e891fecfb58bb98710f9bed1c4d1d2444b1f493e`, and proof is recorded in `test-results/fuse-z80-conformance.json` by commit `2782319`. No cases are skipped; MC/PC helper markers are explicitly excluded from the event comparison because their synthetic contention traces do not model the 48K PAL profile.
...
062. [P2][Core §§44.2,49.1][OPEN] Fuse suite condition is satisfied by run `35918957994`; the gate remains open until project-owned CPU acceptance tests pass, exact bus-cycle traces are demonstrated, and hosted measurement confirms at least eight complete 48K frames retained in the fixed 16 MiB `TIMING_FULL` ring.
...
443. [P16][Core §43] Establish performance baselines for CPU execution, raster generation, audio mixing, tape, snapshots, and UI presentation before optimization — original pre-optimization baselines were captured on `574714b` (run `35886051010`); the accepted raster optimization was recalibrated on `3501819` (run `35888370795`). The attempted four-runner refresh on `b509ee8` (run `35936294325`) failed because all runners reported a raster fingerprint mismatch and is not accepted as baseline evidence. The latest four-runner comparison passed all six workloads with matching deterministic fingerprints on source `269f2cf` (run `35947969719`), and every runner remained within the frozen 40% slowdown limit. The lowest rate was Windows Intel raster generation at 31.3% below baseline; retain this as a remeasurement watch item. Earlier accepted measurements remain in Git history.
...
445. [P16][Core §43] Optimize one subsystem at a time with before/after canonical state/raster/audio regression evidence — completed for canonical raster projection on `3501819`: bitmap and attribute reads are shared across each eight-pixel cell and flash phase is computed once per frame; runner throughput improved 24.1–70.5% versus the original baselines, while raster, audio, tape, snapshot/state, and UI fingerprints remained identical on all four hosted runners (pre-optimization run `35886051010`; post-optimization run `35888370795`). The tracked canonical core regression also pins CPU state, raster, audio, tape, and snapshot/state fingerprints. After timed ULA capture, profile-bound validation, rendered FLASH/BRIGHT coverage, and timestamped beeper/AY clock coverage, the canonical proof passed on run `35947969734`; all six benchmark fingerprints passed on four runners in run `35947969719`.
446. [P16][Core §§7] Reject optimizations that introduce undefined behavior, host-width assumptions, or compiler-specific deterministic results — completed for the raster projection and timed ULA capture: capture row/cell bounds are checked before indexing, 48K PAL profiles exceeding capture capacity are rejected before machine state or allocations are changed, storage is heap allocated and released, and all eight compiler/platform proofs passed on source `269f2cf` (run `35947969742`) with compiler metadata and fixture hashes verified against the pinned Git objects.
447. [P16][Core §§6,31] Reject optimizations that make machine results depend on thread scheduling or host wall clock — completed: raster flash phase and ULA fetch capture are derived from machine profile and master-tick inputs; canonical fingerprints matched across all eight compiler/platform configurations on run `35947969742`, and benchmark fingerprints matched on all four runners on run `35947969719`.
448. [P16][Core §43] Add performance regressions/benchmarks for accepted optimizations so later changes do not silently erase gains — completed: the hosted regression workflow compares six workload fingerprints and runner-specific throughput against frozen four-runner baselines. All fingerprints matched and all throughputs remained within the 40% slowdown limit on run `35947969719`; Windows Intel raster generation was 31.3% below baseline and is recorded for remeasurement.
449. [P16][Core §49.1] Close the Phase-16 gate when every optimization in the release has correctness evidence — completed: canonical regression run `35947969734`, eight compiler/platform proofs on run `35947969742`, and four-runner six-workload performance comparison on run `35947969719` passed for the current implementation; earlier before/after raster evidence remains pinned in run `35888370795`.

## Final Architecture-#1/#2 acceptance and release-readiness sweep

450. [FINAL][Core §50][AUDITED; gaps remain] Inventory all 75 criteria and record conservative evidence status in [acceptance-evidence-audit.md](acceptance-evidence-audit.md); 9 PASS, 14 PARTIAL, and 52 OPEN. The audit is complete; architecture completion remains gated on resolving the OPEN criteria.
451. [FINAL][UI §48][AUDITED; gaps remain] Inventory all 67 criteria in [ui-acceptance-evidence-audit.md](ui-acceptance-evidence-audit.md); current status is 0 PASS, 7 PARTIAL, and 60 OPEN. Resolve the listed proof gaps before declaring UI acceptance.
...
454. [FINAL][Core §§34,35][COMPLETE] The hosted compiler matrix built the product and passed the canonical regression for all eight Core §35 compiler/platform groups on source `269f2cfe4b0cca5fc8c247b7c4e8bd51441c5c37` in run `35947969742`. All eight proof artifacts were inspected; compiler IDs, versions, paths, and generators were present, and every fixture hash matched the pinned Git objects.
455. [FINAL][Core §§36–40][AUDITED; gate open] Deterministic, video, audio, differential, private-media, and hardware evidence remains incomplete as recorded in [final release-readiness audit](final-release-readiness-audit.md) and the criterion audits. Hosted canonical regression on source `269f2cf` passed ULA schedule, timed write/fetch ordering, end-of-frame blanking, border timing, profile-capacity rejection, FLASH/BRIGHT phase plus rendered-pixel checks, and timestamped beeper/AY clock checks (`35947969734`); all eight compiler/platform proofs passed on the same source (`35947969742`), with compiler details and fixture hashes verified. Four-runner performance evidence on the same source passed all six deterministic metric checks (`35947969719`). Core criteria 15–20 remain PARTIAL pending broader frame/video and hardware evidence; criterion 24 passes for the tested timestamped beeper transitions, and criterion 25 is PARTIAL pending wider AY register, envelope, noise, and mixed-output coverage. This change corrects AY envelope direction changes at alternating cycle boundaries and restores the programmed initial envelope level for hold-plus-alternate shapes; canonical regression coverage for triangle, attack/decay hold combinations, noise output, and envelope-controlled mixing is awaiting hosted validation. Automated review findings were adjudicated: a not-yet-due first fetch returns before cursor advancement; boundary assertions render after writes; capture allocations have verified init/destroy/reset and ownership; invalid profiles are rejected before initialization changes machine state or allocates storage, so reuse and cleanup are valid; early failure follows one cleanup path; `wz_machine_memory_write` is declared `void`, so checking a result is inapplicable, while the rendered-pixel assertions verify both writes; and `wz_headless_runner_init` validates its arguments and assigns both runner fields before use, with no required destroy call. Complete video/audio suites, Warajevo differential coverage, private difficult-media completion, and real-hardware/reference certification remain open.
456. [FINAL][Core §§28,47,48][AUDITED; gate open] Release dependency, licensing, ROM/private-media exclusion, and final single-binary artifact evidence is not complete; see [final release-readiness audit](final-release-readiness-audit.md).
457. [FINAL][Core §§28.7][UI §44.1][COMPLETE] Deterministic multi-process stress passed all five cases on Windows x64/ARM64 and macOS Intel/ARM64 in hosted run `35928320489` at `8d5003419b72c3ee53b359d9a6e2dba4f9c86502`; pinned proof: [multi-instance-stress.json](../../test-results/multi-instance-stress.json).
458. [FINAL][Core §§30,55][UI §§4,40,46][COMPLETE] Four hosted runners passed six application-command boundary cases in run `35932389804`, pinned to `1b4a9bc955dcd4a337fb1d0fc1436ead3e33b886`; proof: [application-command-boundary.json](../../test-results/application-command-boundary.json). GUI menu and toolbar, application tests, and Telnet dispatch reset through the shared registry; a host-only View menu operation preserved the initialized canonical machine-state hash; and a worker thread could neither rebind the finalized registry nor dispatch a mutation.
459. [FINAL][Core §§5,6,26,45][COMPLETE] Hosted dependency-boundary audit passed on Linux x86-64 in run `35900244380`, pinned to `a9f08190890c0b9d79f778d10eef87ca523d3df5`; it inspected every core source/include, resolved CMake target sources and link dependencies, `wz_headless`, and forbidden-include negative controls. The passing proof and resolved-target record are retained in the run artifact.
460. [FINAL][Core §49][UI §§49–50][AUDITED; gate open] The core audit records 9 PASS, 14 PARTIAL, and 52 OPEN criteria; the UI audit records 0 PASS, 7 PARTIAL, and 60 OPEN. Tasks 452–453 and 455–457 retain implementation, evidence, or release-gate gaps. See [final release-readiness audit](final-release-readiness-audit.md), [acceptance-evidence-audit.md](acceptance-evidence-audit.md), and [ui-acceptance-evidence-audit.md](ui-acceptance-evidence-audit.md).
