...
454. [FINAL][Core §§34,35][COMPLETE] The hosted compiler matrix built the product and passed the canonical regression for all eight Core §35 compiler/platform groups on source `282a4131435fcd5ead41aca1ab9e5ca4d58e7634` in run `35952794200`. All eight proof artifacts were inspected; compiler IDs, versions, paths, and generators were present, and every fixture hash matched the pinned Git objects.
455. [FINAL][Core §§36–40][AUDITED; gate open] Deterministic, video, audio, differential, private-media, and hardware evidence remains incomplete as recorded in [final release-readiness audit](final-release-readiness-audit.md) and the criterion audits. Updated canonical regression passed ULA schedule, timed write/fetch ordering, end-of-frame blanking, border timing, profile-capacity rejection, FLASH/BRIGHT phase and rendered-pixel checks, timestamped beeper/AY timing, AY envelope shapes, noise sequence/output, fixed-point envelope mixing, divider boundaries, and serialization round-trip preservation on run `35952794177`, source `282a413`; all eight compiler/platform proofs passed on run `35952794200`, with fixture hashes verified. Four-runner performance passed all six metric checks on run `35953015461`; every runner stayed within the 40% limit, with the lowest rate macOS Intel CPU execution at 29.8% below baseline. Criterion 25 now passes; criteria 15–20 remain PARTIAL. Complete video/audio suites, Warajevo differential coverage, private difficult-media completion, and real-hardware/reference certification remain open.
