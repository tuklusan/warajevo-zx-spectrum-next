...
445. [P16][Core §43] Optimize one subsystem at a time with before/after canonical state/raster/audio regression evidence — completed for canonical raster projection on `3501819`: bitmap and attribute reads are shared across each eight-pixel cell and flash phase is computed once per frame; runner throughput improved 24.1–70.5% versus the original baselines, while raster, audio, tape, snapshot/state, and UI fingerprints remained identical on all four hosted runners (pre-optimization run `35886051010`; post-optimization run `35888370795`). The tracked canonical core regression also pins CPU state, raster, audio, tape, and snapshot/state fingerprints. After timed ULA capture, profile-bound validation, rendered FLASH/BRIGHT coverage, timestamped beeper/AY timing, and envelope/noise/mixer regression coverage, the canonical proof passed on run `35950305343`; all six benchmark fingerprints passed on four runners in run `35950305430`.
446. [P16][Core §§7] Reject optimizations that introduce undefined behavior, host-width assumptions, or compiler-specific deterministic results — completed for the raster projection, timed ULA capture, and AY envelope progression: capture row/cell bounds are checked before indexing, 48K PAL profiles exceeding capture capacity are rejected before machine state or allocations are changed, storage is heap allocated and released, and all eight compiler/platform proofs passed on source `cb63f9839e677b12c57016dd3e998852bd8ef6f8` (run `35950305304`) with compiler metadata and fixture hashes verified against the pinned Git objects.
447. [P16][Core §§6,31] Reject optimizations that make machine results depend on thread scheduling or host wall clock — completed: raster flash phase, ULA fetch capture, and AY tone/noise/envelope advancement are derived from machine profile and master-tick inputs; canonical fingerprints matched across all eight compiler/platform configurations on run `35950305304`, and benchmark fingerprints matched on all four runners on run `35950305430`.
448. [P16][Core §43] Add performance regressions/benchmarks for accepted optimizations so later changes do not silently erase gains — completed: the hosted regression workflow compares six workload fingerprints and runner-specific throughput against frozen four-runner baselines. All fingerprints matched and all throughputs remained within the 40% slowdown limit on run `35950305430`; Windows ARM64 tape playback was 31.6% below baseline and is recorded for remeasurement.
449. [P16][Core §49.1] Close the Phase-16 gate when every optimization in the release has correctness evidence — completed: canonical regression run `35950305343`, eight compiler/platform proofs on run `35950305304`, and four-runner six-workload performance comparison on run `35950305430` passed for the current implementation; earlier before/after raster evidence remains pinned in run `35888370795`.
...
454. [FINAL][Core §§34,35][COMPLETE] The hosted compiler matrix built the product and passed the canonical regression for all eight Core §35 compiler/platform groups on source `cb63f9839e677b12c57016dd3e998852bd8ef6f8` in run `35950305304`. All eight proof artifacts were inspected; compiler IDs, versions, paths, and generators were present, and every fixture hash matched the pinned Git objects.
455. [FINAL][Core §§36–40][AUDITED; gate open] Deterministic, video, audio, differential, private-media, and hardware evidence remains incomplete as recorded in [final release-readiness audit](final-release-readiness-audit.md) and the criterion audits. Hosted canonical regression on source `cb63f98` passed ULA schedule, timed write/fetch ordering, end-of-frame blanking, border timing, profile-capacity rejection, FLASH/BRIGHT phase plus rendered-pixel checks, timestamped beeper/AY clock checks, AY envelope shapes, noise sequence/output, and fixed-point envelope mixing (`35950305343`); all eight compiler/platform proofs passed on the same source (`35950305304`), with compiler details and fixture hashes verified. Four-runner performance evidence on that source passed all six deterministic metric checks (`35950305430`); Windows ARM64 tape playback was 31.6% below baseline. The focused AY tests exposed that the implementation omitted AY input-clock prescalers despite passing its earlier coarse time checks. Core tone edges now use the documented divide-by-8 counter; noise shifts use divide-by-16; envelope steps use divide-by-256. These phases are added to portable machine-state serialization, and the canonical regression now checks divider boundaries and snapshot round-trip preservation. Hosted validation is pending; criterion 25 remains PARTIAL. Automated review findings were adjudicated: the AY test state is initialized by `wz_ay_init` before reads; the AY diagnostic reuses one getter result; the period-one noise output starts changing after 17 noise shifts; earlier ULA ordering, profile validation, capture ownership/cleanup, void memory-write API, and runner initialization findings remain as recorded. Complete video/audio suites, Warajevo differential coverage, private difficult-media completion, and real-hardware/reference certification remain open.
