...
Task 450 audit is complete; architecture acceptance is not. `PASS` means the cited evidence directly covers the full criterion; `PARTIAL` records narrower evidence; `OPEN` means no adequate tracked proof was found. Source inspection and general builds do not count as behavioral proof. On source commit `e891fec`, all 1,356 cases in the pinned Fuse Z80 suite passed on hosted run `35918957994`; the proof is tracked in `test-results/fuse-z80-conformance.json`. The test-ledger gate passed with that proof on commit `2782319` (run `35919046699`), and the pathname check passed (run `35919046665`). The ledger also contains `canonical-core-regression`; neither suite establishes every behavioral criterion below. Every OPEN criterion remains an unresolved proof or implementation gap, not an implied pass.
...
| 15 | the ULA fetches memory on an explicit raster schedule; | PARTIAL | `ula_fetch_schedule` in `canonical-core-regression` passed on hosted run `35938173613`, pinned to source commit `9451f78`. It checks the 48K PAL first bitmap/attribute fetch ticks and values, consecutive cell and scanline addresses, fetch-window end, and floating-bus values. Full-frame schedule and hardware/reference correlation remain open. |
...
| 17 | full border/raster timing is represented; | PARTIAL | `src/core/wz_machine.c` maps the 48K PAL active region from frame line 64 and projects timestamped border events across the raster. `border_event_timing` in the canonical regression verified timestamp recording, adjacent-tick changes, right/left border positions, and active-pixel preservation on hosted run `35937597748` (source commit `658d2e5`); the canonical raster fingerprint and all eight compiler/platform groups also passed on that source commit (run `35937597769`). Broader full-frame raster/fetch timing evidence remains required. |
...
| 51 | the complete pinned Fuse Z80 unit-test suite passes with no unexplained skipped or failing applicable cases; | PASS | All 1,356 cases passed on hosted Linux x86-64 run `35918957994`, pinned to Fuse revision `c94a5e611bb0b795d62e904a0f5af4f59f2a4eee`; the proof is tracked at `test-results/fuse-z80-conformance.json`. No cases are skipped. Synthetic MC/PC contention-helper markers are explicitly excluded from this CPU event comparison. |
