...
| 455 | The tracked ledger contains the canonical core regression and pinned Fuse suite. Updated canonical regression passed on source `282a4131435fcd5ead41aca1ab9e5ca4d58e7634` in run `35952794177`; all eight compiler/platform groups passed on run `35952794200`. Four-runner performance passed all six workload fingerprints and the 40% slowdown gate in run `35953015461`; macOS Intel CPU execution was the lowest throughput at 29.8% below baseline. State format v14 now preserves AY tone divide-by-8, noise divide-by-16, and envelope divide-by-256 phases; hosted regression covers the boundaries and snapshot round trip. Core criteria 15–20 remain PARTIAL, criterion 24 passes, and criterion 25 now passes. Complete video/audio suites, differential cases, private media, and hardware/reference certification remain open. | Add the remaining architecture-required suites on hosted infrastructure; record unavailable private corpus or hardware evidence explicitly and do not claim the corresponding release scope without it. |
