...
| 455 | The tracked ledger contains the canonical core regression and pinned Fuse suite. Hosted run `35950305343` passed the current pre-prescaler video/audio regression set on source `cb63f9839e677b12c57016dd3e998852bd8ef6f8`; all eight compiler/platform groups passed on run `35950305304`, and four-runner performance passed on run `35950305430`. Review of the AY timing contract showed that those tone/noise/envelope checks lacked chip input-clock prescalers. The core now adds divide-by-8 tone, divide-by-16 noise, and divide-by-256 envelope phases, persists them in state serialization, and tests each boundary and serialization round-trip. Updated hosted validation is pending. Existing criteria 15–20 remain PARTIAL, criterion 24 passes, and criterion 25 is PARTIAL until the corrected implementation has hosted proof. | Run the updated canonical regression and all compiler/platform checks; continue with broader frame/audio suites, differential cases, private media, and hardware/reference certification. |
