...
| 455 | The tracked test ledger contains `canonical-core-regression` and the 1,356-case pinned Fuse Z80 suite. Fuse CPU conformance passed on hosted run `35918957994` (proof commit `2782319`). The 48K PAL renderer now projects recorded border events over frame coordinates; the canonical raster fingerprint and compiler matrix passed on `b509ee8` (runs `35936294351`, `35936294434`), but those fixtures contain no border transitions. Complete video/audio suites, a border-event timing regression, Warajevo differential coverage, private difficult-media completion, and real-hardware/reference certification remain open. Core and UI acceptance audits enumerate additional gaps. | Add/execute the remaining architecture-required suites on hosted infrastructure; record unavailable private corpus or hardware evidence explicitly and do not claim the corresponding release scope without it. |
...
| 460 | The core audit records 8 PASS, 7 PARTIAL, and 60 OPEN criteria; the UI audit records 0 PASS, 7 PARTIAL, and 60 OPEN. Tasks 452–453 and 455–457 retain implementation, evidence, or release-gate gaps. | Close each applicable phase gate and acceptance criterion with pinned hosted evidence. Preserve deferred scope explicitly and resolve architecture decisions before implementation claims. |
