...
| 455 | The tracked ledger contains the canonical core regression and pinned Fuse suite. Updated canonical regression on source `0aa7958f0a3d45c86d86507de2423087191f6ea1` passed full-frame fetch, floating-bus, and border-raster coverage plus timing/audio checks in run `35955514192`; all eight compiler/platform groups passed on run `35955514187`, with fixture hashes verified. Four-runner performance passed all six workload fingerprints and the 40% slowdown gate in run `35955514283`; macOS Intel UI presentation was lowest at 38.7% below baseline. State format v14 preserves AY tone divide-by-8, noise divide-by-16, and envelope divide-by-256 phases, with hosted boundary and snapshot-round-trip coverage. Sokol host pacing now executes profile-sized frame batches and applies speed changes at host pacing anchors; the four-runner DIZZY4K smoke accepted `SPEED 800` while running and completed in run `35957300837`, source `dd512af`. This does not measure speed ratios or prove master-tick continuity, and host audio output remains unconnected. Core criteria 18, 24, and 25 pass; 15–17 and 19–20 remain PARTIAL. Complete video/audio suites, differential cases, private media, and hardware/reference certification remain open. | Add timing and host-audio regression evidence plus remaining architecture-required suites on hosted infrastructure; record unavailable private corpus or hardware evidence explicitly and do not claim the corresponding release scope without it. |
