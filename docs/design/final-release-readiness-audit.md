...
| 459 | Static inspection found no forbidden host includes/calls in `src/core`; `wz_core` and `wz_headless` retain the required dependency direction. Added `ci/test/core-boundary/check.py` and a main-only workflow with negative include controls; hosted proof is pending. | Require a passing main-branch run and inspect its pinned proof artifact; retain mandatory compiler-matrix coverage as a separate Task 454 gate. |
