                         ...
                         wz_input_arbiter_t* arbiter)
{
    if (controller != 0) {
        ...
        controller->target = WZ_INPUT_FOCUS_VIEWPORT;
    }
}
...
{
    if (controller == 0 ||
        (target != WZ_INPUT_FOCUS_VIEWPORT &&
         target != WZ_INPUT_FOCUS_TEXT_CONTROL)) {
        return false;
    }
    if (target == WZ_INPUT_FOCUS_TEXT_CONTROL && controller->focused) {
        if (controller->arbiter == 0 ||
...
wz_input_focus_target_t wz_input_focus_target(
    const wz_input_focus_controller_t* controller)
{
    return controller == 0 ? WZ_INPUT_FOCUS_TEXT_CONTROL : controller->target;
}

bool wz_input_focus_dialog_enter(wz_input_focus_controller_t* controller)
{
    return wz_input_focus_set_target(controller, WZ_INPUT_FOCUS_TEXT_CONTROL);
}

bool wz_input_focus_dialog_leave(wz_input_focus_controller_t* controller)
{
    return wz_input_focus_set_target(controller, WZ_INPUT_FOCUS_VIEWPORT);
}
