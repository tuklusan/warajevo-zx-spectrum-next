...
bool wz_host_output_write_exclusive(const char* path,
                                    const void* data,
                                    size_t size)
{
    HANDLE file;
    ...
    success = write_complete(file, data, size) && CloseHandle(file);
    if (!success) {
                                    ...
                                    size_t size)
{
    ...
    success = write_complete(file, bytes, size) && close(file) == 0;
