/*
Copyright (c) 2026 Supratim Sanyal of SANYALnet Labs.
This file is governed by the SANYALnet Labs Non-Commercial License in the
root LICENSE file. Non-Commercial use is permitted; Commercial Use and use
for AI/ML model training are prohibited unless separately authorized.
Attribution is required: "Based on original work by Supratim Sanyal of
SANYALnet Labs." See LICENSE for full terms.
*/

#ifndef WZ_APP_WZ_UI_LAYOUT_H
#define WZ_APP_WZ_UI_LAYOUT_H

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>

#include "app/wz_speed_policy.h"
#include "app/wz_control_port.h"
#include "app/wz_command_registry.h"
#include "core/wz_machine.h"

#define WZ_UI_MENU_COUNT 7u
#define WZ_UI_TOOLBAR_COUNT 11u
#define WZ_UI_MENU_BAR_HEIGHT 28.0f
#define WZ_UI_TOOLBAR_HEIGHT 28.0f
#define WZ_UI_TAPE_ACTION_COUNT 5u
#define WZ_UI_MICRODRIVE_ACTIONS_PER_SLOT 3u
#define WZ_UI_MICRODRIVE_ACTION_COUNT \
    (WZ_UI_MICRODRIVE_COUNT * WZ_UI_MICRODRIVE_ACTIONS_PER_SLOT)
#define WZ_UI_STATUS_CAPACITY 256u
#define WZ_UI_REMOTE_STATUS_CAPACITY 512u
#define WZ_UI_MICRODRIVE_COUNT 8u
#define WZ_UI_MICRODRIVE_NAME_CAPACITY 64u
#define WZ_UI_NETWORKING_OPTION_COUNT 3u
#define WZ_UI_NETWORKING_COMMAND_ID "machine.networking.set"
#define WZ_UI_NETWORKING_EAR_MIC_DISABLED_REASON \
    "requires-architecture-3-certification"

typedef wz_command_menu_root_t wz_ui_menu_node_t;

typedef struct {
    const char* command_id;
    const char* label;
} wz_ui_toolbar_item_t;

typedef struct {
    wz_networking_mode_t mode;
    const char* command_id;
    const char* label;
    bool available;
    const char* disabled_reason;
} wz_ui_networking_option_t;

typedef enum {
    WZ_UI_REMOTE_LISTENER_UNAVAILABLE = 0,
    WZ_UI_REMOTE_LISTENER_DOWN,
    WZ_UI_REMOTE_LISTENER_UP,
    WZ_UI_REMOTE_LISTENER_DEGRADED
} wz_ui_remote_listener_state_t;

typedef struct {
    bool service_enabled_by_architecture;
    unsigned base_control_port;
    unsigned probe_first_port;
    unsigned probe_last_port;
    bool selected_control_port_available;
    unsigned selected_control_port;
    bool ipv4_listener_up;
    bool ipv6_listener_up;
    wz_ui_remote_listener_state_t listener_state;
    bool active_client;
    bool plaintext_no_authentication;
    const char* permission_summary;
} wz_ui_remote_control_status_t;

typedef enum {
    WZ_UI_FOCUS_VIEWPORT = 0,
    WZ_UI_FOCUS_MENU,
    WZ_UI_FOCUS_DIALOG,
    WZ_UI_FOCUS_STATUS
} wz_ui_focus_target_t;

typedef struct {
    wz_ui_focus_target_t focus_target;
    bool keyboard_operable;
    bool visible_focus;
    bool non_color_status;
} wz_ui_keyboard_status_t;

typedef struct {
    const char* toolkit;
    const char* toolkit_revision;
    bool project_semantics_enabled;
    bool native_adapter_available;
    const char* native_adapter_state;
} wz_ui_accessibility_descriptor_t;

typedef enum {
    WZ_UI_MICRODRIVE_VALIDATION_UNMOUNTED = 0,
    WZ_UI_MICRODRIVE_VALIDATION_VALID,
    WZ_UI_MICRODRIVE_VALIDATION_INVALID,
    WZ_UI_MICRODRIVE_VALIDATION_UNAVAILABLE
} wz_ui_microdrive_validation_t;

typedef struct {
    bool mounted;
    uint64_t host_image_identity;
    char logical_name[WZ_UI_MICRODRIVE_NAME_CAPACITY];
    size_t sector_count;
    bool write_protected;
    bool current_drive;
    bool default_drive;
    wz_ui_microdrive_validation_t validation;
} wz_ui_microdrive_overview_entry_t;

typedef struct {
    unsigned model_k;
    unsigned speed_percent;
    bool unlimited_speed;
    bool paused;
    bool audio_muted;
    bool tape_mounted;
    bool microdrive1_mounted;
    bool microdrive_mounted[WZ_UI_MICRODRIVE_COUNT];
    bool fullscreen;
    bool status_panel_visible;
    wz_networking_mode_t networking_selection;
    const char* networking_mode;
    bool remote_control_enabled;
    bool remote_control_client_connected;
    bool control_port_available;
    unsigned control_port;
    wz_ui_remote_listener_state_t remote_listener_state;
} wz_ui_layout_state_t;

void wz_ui_layout_state_init(wz_ui_layout_state_t* state);
size_t wz_ui_layout_menu_count(void);
const wz_ui_menu_node_t* wz_ui_layout_menu_at(size_t index);
size_t wz_ui_layout_toolbar_count(void);
const wz_ui_toolbar_item_t* wz_ui_layout_toolbar_at(size_t index);
bool wz_ui_layout_toolbar_hit_test(float x, float y,
                                   float viewport_width,
                                   size_t* toolbar_index);
size_t wz_ui_layout_tape_action_count(void);
const wz_ui_toolbar_item_t* wz_ui_layout_tape_action_at(size_t index);
size_t wz_ui_layout_microdrive_action_count(void);
const wz_ui_toolbar_item_t* wz_ui_layout_microdrive_action_at(size_t index);
size_t wz_ui_layout_networking_option_count(void);
const wz_ui_networking_option_t* wz_ui_layout_networking_option_at(
    size_t index);
bool wz_ui_layout_select_networking(wz_ui_layout_state_t* state,
                                    wz_networking_mode_t mode);
wz_result_t wz_ui_layout_activate_networking(
    const wz_command_registry_t* registry,
    wz_networking_mode_t mode,
    wz_command_result_t* result);
void wz_ui_layout_microdrive_overview_init(
    wz_ui_microdrive_overview_entry_t* entries);
size_t wz_ui_layout_microdrive_overview_count(void);
const wz_ui_microdrive_overview_entry_t* wz_ui_layout_microdrive_overview_at(
    const wz_ui_microdrive_overview_entry_t* entries,
    size_t index);
bool wz_ui_layout_microdrive_overview_set(
    wz_ui_microdrive_overview_entry_t* entries,
    size_t index,
    uint64_t host_image_identity,
    const char* logical_name,
    size_t sector_count,
    bool write_protected,
    bool current_drive,
    bool default_drive,
    wz_ui_microdrive_validation_t validation);
const char* wz_ui_layout_microdrive_validation_label(
    wz_ui_microdrive_validation_t validation);
void wz_ui_layout_tape_label(bool mounted, char* output, size_t capacity);
void wz_ui_remote_control_status_init(wz_ui_remote_control_status_t* status);
void wz_ui_remote_control_status_page(
    const wz_ui_remote_control_status_t* status,
    char* output,
    size_t capacity);
void wz_ui_keyboard_status_init(wz_ui_keyboard_status_t* status);
void wz_ui_keyboard_status_page(const wz_ui_keyboard_status_t* status,
                               char* output,
                               size_t capacity);
void wz_ui_accessibility_descriptor_init(
    wz_ui_accessibility_descriptor_t* descriptor);
void wz_ui_accessibility_descriptor_page(
    const wz_ui_accessibility_descriptor_t* descriptor,
    char* output,
    size_t capacity);
void wz_ui_layout_status_line(const wz_ui_layout_state_t* state,
                              char* output,
                              size_t capacity);
void wz_ui_layout_status_panel(const wz_ui_layout_state_t* state,
                               char* output,
                               size_t capacity);
void wz_ui_layout_apply_control_port(
    wz_ui_layout_state_t* state,
    const wz_control_port_owner_t* owner);
void wz_ui_layout_sync_remote_control(
    wz_ui_layout_state_t* state,
    wz_ui_remote_control_status_t* remote_status,
    const wz_control_port_owner_t* owner,
    bool active_client);
bool wz_ui_layout_toggle_status_panel(wz_ui_layout_state_t* state);
size_t wz_ui_layout_speed_count(void);
const char* wz_ui_layout_speed_label(size_t index);
bool wz_ui_layout_select_speed(wz_ui_layout_state_t* state,
                               wz_speed_policy_t speed);
wz_result_t wz_ui_layout_activate_toolbar(
    const wz_command_registry_t* registry,
    size_t index,
    wz_command_arguments_t arguments,
    wz_command_result_t* result);
wz_result_t wz_ui_layout_activate_tape_action(
    const wz_command_registry_t* registry,
    size_t index,
    wz_command_arguments_t arguments,
    wz_command_result_t* result);
wz_result_t wz_ui_layout_activate_microdrive_action(
    const wz_command_registry_t* registry,
    size_t index,
    wz_command_arguments_t arguments,
    wz_command_result_t* result);
wz_command_state_t wz_ui_layout_command_state(
    const wz_command_registry_t* registry,
    const char* command_id,
    const char** disabled_reason);

#endif
