                            ...
                            const wz_machine_profile_t* profile)
{
    ...
    machine->debugger_breakpoint_address = 0u;
                                                ...
                                                bool inclusive)
{
    ...
    for (;;) {
        ...
        if ((wz_qword_t)machine->ula_capture_event_index >= events_per_frame) {
            ...
            /* Keep the cursor on this frame through trailing blanking. */
                                     ...
                                     wz_raster_buffer_t* destination)
{
    ...
    for (size_t y = 0u; y < 192u; ++y) {
        ...
        wz_word_t attribute_row = (wz_word_t)(0x5800u + (row / 8u) * 32u);
        for (size_t cell = 0u; cell < 32u; ++cell) {
            wz_word_t bitmap_address = wz_ula_bitmap_address(
                row, (wz_dword_t)cell);
            wz_word_t attribute_address = (wz_word_t)(attribute_row + cell);
