...
int main(void)
{
    wz_command_metadata_t storage[10];
    ...
    assert(wz_microdrive_manager_operation_count() == 10u);
    ...
    assert(wz_microdrive_manager_operation_at(10u) == 0);

    assert(wz_command_registry_init(&registry, storage, 10u) == WZ_RESULT_OK);
    ...
    assert(wz_command_registry_count(&registry) == 10u);
